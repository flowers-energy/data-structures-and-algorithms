...
const toLastNames = people => {
  ...
  const peopleArr = people.map(person => person.firstName + ' ' + person.lastName);
  return peopleArr;
};
...
const addValues = (arr) => arr.reduce((acc, cur) => {
  acc += cur;
  return acc;
}, 0);
...
const addPurchases = (arr) => arr.reduce((acc, curr) => acc += curr.purchasePrice, 0);
